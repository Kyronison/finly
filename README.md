...
- `TINKOFF_TERMINAL_KEY` и `TINKOFF_SECRET_KEY` — ключи терминала для формирования платёжных ссылок Тинькофф.
- `SBER_USERNAME` и `SBER_PASSWORD` — логин и пароль мерчанта Сбер для REST API регистрации заказа (опционально можно задать `SBER_ENDPOINT`).
